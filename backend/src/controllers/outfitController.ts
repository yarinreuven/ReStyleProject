...
export async function createTryOn(
  ...
  next: NextFunction
    ...
    try {
      ...
      try {
        let generated: Awaited<ReturnType<typeof createGeminiTryOnImage>> | null = null;
        let quality = generatedImageAcceptedForUserReview(orderedSelectionItems);
        for (let generationAttempt = 1; generationAttempt <= 2; generationAttempt += 1) {
          generated = await createGeminiTryOnImage(
            avatar.data,
            avatar.contentType,
            tryOnInputs
          );
          logger.info({
            ...
            generationAttempt
          }, "Gemini try-on image completed");
          try {
            quality = await validateGeminiTryOnImage(
              generated.data,
            ...
            // The generated bytes have already passed the local image integrity
            // checks. Do not discard a completed try-on only because the separate
            // quality-checking model is temporarily unavailable or malformed.
            quality = generatedImageAcceptedForUserReview(orderedSelectionItems);
            logger.warn(validationError instanceof GeminiTryOnServiceError
              ? {
                  stage: validationError.stage,
                  code: validationError.code,
                  httpStatus: validationError.httpStatus,
                  providerStatus: validationError.providerStatus
                }
              : {
                  stage: "validation",
                  code: validationError instanceof Error && validationError.name === "TimeoutError"
                    ? "TIMEOUT"
                    : "UNEXPECTED_VALIDATION_ERROR"
                }, "Gemini try-on quality check unavailable; using verified local fallback");
          }
          const qualityError = qualityValidationError(quality, orderedSelectionItems);
          if (!qualityError) break;
          if (generationAttempt === 2) {
            throw new Error(`TRY_ON_QUALITY_REJECTED: ${qualityError}`);
          }
          logger.warn({
            generationAttempt,
            reason: qualityError
          }, "Gemini try-on quality rejected; regenerating within the same request");
        }
        if (!generated) throw new Error("TRY_ON_IMAGE_NOT_GENERATED");
